<template>
    <div class="login_body">
                ...
                <div>
                    <input type="text" class="login_text" v-model="verifyImg" placeholder="请输入您的验证码"><img @touchstart="handleToVerifyImg" src="/api3/users/verifyImg">
                </div>
                <div class="login_btn">
                    <input @click="handleToLogin()" type="submit" value="登录">
                </div>
                <div class="login_link">
                    <router-link tag="a" to="/my/register">立即注册</router-link>
                    <router-link tag="a" to="/my/findpassword">找回密码</router-link>
                </div>
            </div>
</template>
<script>
